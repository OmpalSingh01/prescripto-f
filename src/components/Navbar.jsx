  ...
  return (
    <>
      ...
      <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-[#ADADAD]'>
        ...
        {/* --- Mobile Menu --- */}
        <div className={`md:hidden fixed top-0 right-0 h-full bg-white z-20 overflow-hidden transition-all ${showMenu ? 'w-64' : 'w-0'}`}>
          <div className='flex items-center justify-between px-5 py-6'>
            <img src={assets.logo} className='w-36' alt="" />
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
            {navLinks.map((link, idx) => (
              <NavLink key={idx} onClick={() => setShowMenu(false)} to={link.path}>
                <p className='px-4 py-2 rounded-full inline-block'>{link.label}</p>
              </NavLink>
            ))}
            {!token && (
              <button 
      ...
      {/* --- Login Modal with Fade & Scale Animation --- */}
      {showLoginModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          {/* Modal Content */}
          <div className="relative bg-white rounded-lg p-6 w-96 shadow-xl transform transition-all duration-300 scale-95 opacity-0 animate-fadeIn">
            <button 
              onClick={() => setShowLoginModal(false)} 
